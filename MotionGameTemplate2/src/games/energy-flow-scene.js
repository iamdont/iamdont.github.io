// src/games/energy-flow-scene.js (最終修正版)
// [FINAL FIX] 使用獨立平滑器，並智能選擇左右手，提升穩定性

import { BaseScene } from '../core/base-scene.js';

// 同 path-of-balance 一樣，用一個場景內部嘅極簡平滑器
class SimplePointSmoother {
    constructor(smoothingFactor = 0.85) { // 呢個 game 需要更高嘅平滑度
...
export class EnergyFlowScene extends BaseScene {
    constructor() {
        ...
        // 為左右手創建獨立平滑器
        this.handSmoothers = {
            left: new SimplePointSmoother(),
            right: new SimplePointSmoother(),
        };
    ...
    onInit() {
        ...
        this.motionEngine.showPointer(true); // 顯示指針以便操作 UI
        this.motionEngine.setSmoothing(0.8); // 為 UI 指針設置高平滑度
    ...
    onUpdate() {
        ...
        const distToHand = this.hand.isVisible ? Math.hypot(this.hand.pos.x - this.orb.pos.x, this.hand.pos.y - this.orb.pos.y) : Infinity;
    ...
    startGame() {
        ...
        this.handSmoothers.left.reset();
        this.handSmoothers.right.reset();
    ...
    updateHandPosition() {
        ...
        const rawLeft = (landmarks[15] && landmarks[15].visibility > visibilityThreshold) ? window.getPalmCenterFromPose(landmarks, 'left') : null;
        const rawRight = (landmarks[16] && landmarks[16].visibility > visibilityThreshold) ? window.getPalmCenterFromPose(landmarks, 'right') : null;

        const smoothedLeft = this.handSmoothers.left.update(rawLeft);
        const smoothedRight = this.handSmoothers.right.update(rawRight);
        
        let activeHandPoint = null;
        
        // [FINAL FIX] 智能揀手邏輯
        if (smoothedLeft && smoothedRight) {
            // 如果兩隻手都喺度，揀離能量球近啲嗰隻
            const leftDist = Math.hypot((1 - smoothedLeft.x) * this.canvas.width - this.orb.pos.x, smoothedLeft.y * this.canvas.height - this.orb.pos.y);
            const rightDist = Math.hypot((1 - smoothedRight.x) * this.canvas.width - this.orb.pos.x, smoothedRight.y * this.canvas.height - this.orb.pos.y);
            activeHandPoint = (leftDist < rightDist) ? smoothedLeft : smoothedRight;
        } else {
            // 否則，用存在嘅嗰隻手
            activeHandPoint = smoothedLeft || smoothedRight;
        }

        if (activeHandPoint) {
            this.hand.isVisible = true;
            this.hand.prevPos = { ...this.hand.pos };
            this.hand.pos.x = (1 - activeHandPoint.x) * this.canvas.width;
            this.hand.pos.y = activeHandPoint.y * this.canvas.height;
